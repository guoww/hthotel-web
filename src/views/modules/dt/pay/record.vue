  ...
  <div class="mod-role">
    ...
    <el-table
      ...
      <el-table-column
        ...
        label="交易类型">
        <template slot-scope="scope">
          <el-tag v-if="scope.row.type==0">订单支付</el-tag>
          <el-tag v-else-if="scope.row.type==1" type="danger">订单退款</el-tag>
          <el-tag v-else-if="scope.row.type==2">会员卡购买</el-tag>
          <el-tag v-else-if="scope.row.type==3">会员充值</el-tag>
        </template>
      </el-table-column>
      <el-table-column
        header-align="center"
  ...
  export default {
    ...
    methods: {
      ...
      getDataList () {
        ...
        this.$http({
          url: this.$http.adornUrl('/dt/payRD/getPage'),
          method: 'get',
